        ...
        for key, value in status_data.items():
            try:
                ...
                if process.is_running():
                    ...
                    prev_ram_str = value.get("RAM", "0 GB").replace(" GB", "")
                    try:
                        prev_ram_mb = float(prev_ram_str) * 1024
                    except ValueError:
                        prev_ram_mb = 0.0
                    threshold = (1 + ram_allowance) * prev_ram_mb if ram_allowance <= 1 else 1.25 * prev_ram_mb
                    if ram_mb > threshold:
                        logging.warning(f"The ram usage of the model was previously: {value.get('RAM')} and currently its : {ram_mb:.2f} MB")
